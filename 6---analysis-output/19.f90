...
Program my_planetcircular
 ...
 INTEGER i,ISEED,idum ,seed,COUNTER,NBP
 INTEGER :: time,STIME
 INTEGER, DIMENSION (9) :: T
 REAL :: RANF,ecc,r_p,ra,a,v_p,vpx0,vpy0,vpz0,Ep,Ek,Energy
 INTEGER N
 PARAMETER(N=3)
 REAL x(N),f(N)
 LOGICAL check
 common/constant/ x0,y0,z0,vx0,vy0,vz0,vc
 !NUMBER_OF_BIND_PLANETS=NBP
 NBP=10
!                            Random Number
 ...
 !call random_seed(put=seed2)
  ...
  open(3,file='Ep-Ek-E.txt')
  	COUNTER=0
  Do i=1,500
   IF(COUNTER .LT. NBP) THEN
	...
	!a=r_p/(1.-ecc)
	r_p=a*(1.-ecc)
	!r_p=sqrt(x0**2+y0**2+z0**2)
	 x0=r_p*sin(theta)*cos(phi)
	 y0=r_p*sin(theta)*sin(phi)
	 z0=r_p*cos(theta)
...
! vx*rx+vy*ry+vz*rz=0.
	...
	!BINDING ENERGY CHECK
	 Ep=-G*m_p*M_star/r0
     Ek=0.5*m_p*(Sqrt(vx0**2+vy0**2+vz0**2))**2.
     Energy=Ep+Ek
   IF(Energy .LT. 0.)THEN 
